...
1. 超高流量下怎么保证系统稳定性，防止并发过高造成缓存击穿，数据库宕机
2. 怎么保证数据最终一致性，比如库存不能超卖
3. 接口防刷
4. 数据库设计，就算发生秒杀宕机，不影响其他业务
5. 承载巨大访问量的同时提供低延时服务

### 系统优化方案

1. 首先收集系统信息，当前流量多少，都用了哪些中间件，当前tp99是多少，什么服务器，服务器什么配置，内存多大，是不是ssd，运行程序占多大的cpu，需要把这些信息收集好，而不是上来就去优化数据库或代码。
2. 需要了解整个系统的能产生瓶颈的点，从网页或app加载开始，请求进入后一直到收到返回，都需要经历哪些服务，把有可能产生瓶颈的地方都列出来，最好有trace来分析整个流程。

## 领域驱动

可以参考这个：[殷浩详解DDD：如何避免写流水账代码](https://mp.weixin.qq.com/s/SjU1DbsXcBD-2DJt9z65zg)

### 领域建模

领域建模的目的是统一语言，让开发和产品减少沟通成本，领域是对业务的抽象，业务流程和业务场景通过抽象，划定出相关的业务功能，高内聚低耦合，职责单一，符合开闭原则
...
地址：https://blog.csdn.net/xinzhifu1/article/details/109727873
...
2. 单线程操作，避免了频繁的上下文切换
3. 合理高效的数据结构
5. 采用了非阻塞I/O多路复用机制（有一个文件描述符同时监听多个文件描述符是否有数据到来）

### Redis 的底层数据sds结构

 一个 SDS 值的数据结构，主要由 len、free、buf[] 这三个属性组成。 [面试官：Redis 为什么把简单的字符串设计成 SDS？](https://mp.weixin.qq.com/s/WrNqy5l3nN5Yxe3Xqk3vgw)
...
2.  当我们需要修改数据时，首先会检查当前 SDS 空间 len 是否满足，不满足则自动扩容空间至修改所需的大小，然后再执行修改。 

3. 数据多样性，c字符串不能包含\0，而redis可以存二进制的数据比如图片等

4. 内存重分配策略

   1. 空间预分配：当修改字符串并需对SDS的空间进行扩展时，不仅会为 SDS 分配修改所必要的空间，还会为 SDS 分配额外的未使用空间 free，下次再修改就先检查未使用空间 free 是否满足，满足则不用在扩展空间。 对sds字符串修改，len值小于1m，则free空间与len相等，如果大于1m，则free等于1m![1m](https://mmbiz.qpic.cn/mmbiz_png/0OzaL5uW2aPeeALwPsaGl6DjfgBvPspibHXGHQltYYA7ouj2XIrhictAy7VGpYQnCp9WwuwDwIG3yOmYoxyvrTJg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
   2. 惰性空间释放： 当缩短 SDS 字符串后，并不会立即执行内存重分配来回收多余的空间，而是用 free 属性将这些空间记录下来。如果后续有增长操作，则可直接使用。  ![img](https://mmbiz.qpic.cn/mmbiz_png/0OzaL5uW2aPeeALwPsaGl6DjfgBvPspibRYiaWibPqWtL8qBqnwmicEBuMundaXEDnCp2mfwiaxCNPdSKLKeBXxBRGg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1) 
...
1. 主从复制
2. 哨兵模式，集群中有多个哨兵节点，向master和其他哨兵发送ping请求，当回复时间大于配置时间，则判断主观下线，这时其他哨兵会1秒一次的请求，如果多数都收到超时也是可配置，那判断客观下线，如果下线节点重新发送有效回复，下线状态移出
3. cluster模式，结合了主从和哨兵，一个slot是虚拟节点，一致性hash
...
先更新数据库，再删缓存。数据库的读操作的速度远快于写操作的，所以脏数据很难出现。可以对异步延时删除策略，保证读请求完成以后，再进行删除操作。[美团二面：Redis与MySQL双写一致性如何保证？](https://mp.weixin.qq.com/s/ZTCbrR65Xyr23xAp36_wcw)
...
[redis分布式锁看这篇就够了](https://mp.weixin.qq.com/s/EGGe14IpEsho75ntjeR3OA)

基本还是使用setnx来实现，看哪个线程可以返回1相当于加锁成功，在程序处理完成之后再把key删除，但有个问题，如果程序失败导致锁一直无法释放，所以要加过期时间，在 2.6.12 版本之前需要单独设置，在这之后使用set命令即可。
...
例如，操作共享资源的时间「最慢」可能需要 15s，而我们却只设置了 10s 过期，那这就存在锁提前过期的风险。  既然是「预估」时间，也只能是大致计算，除非你能预料并覆盖到所有导致耗时变长的场景，但这其实很难。 
...
但是这里先查再删，可能会出现线程安全问题，所以这里要使用lua脚本
...
当redis做缓存时，对key取模把数据分散到redis集群，当redis集群数量变更时大量的key用新取模算法无法找到缓存，一致性hash的想法是构建一个环，key取模后顺时针指向最近的节点，当节点挂点后只影响一个节点的数据。为了解决数据倾斜的问题，引入虚拟节点，分散请求，对于读多写少的情况适合用二叉平衡树，红黑树存储hash环。[一致性 Hash 在负载均衡中的应用](https://www.cnblogs.com/fengyun2050/p/12808951.html)，[一致性Hash原理与实现](https://www.jianshu.com/p/528ce5cd7e8f)。

## Mysql

一个非常好的mysql学习手册 [《MySQL实战45讲》--读书笔记](https://blog.csdn.net/u010657094/article/details/100173921)
...
可重复读 repeatable read ：mysql默认级别，两次读取的数据一致，使用mvcc解决
...
[mysql的count(*)性能真的不好吗？](https://bbs.huaweicloud.com/blogs/323789)

结论count(*) = count(1) > count(主键字段) > count(字段)

count函数说明：**统计符合查询条件的记录中，函数指定的参数不为NULL的记录有多少个**

count(*)和count(1)执行过程一样，mysql会将 * 转化为0处理，而且mysql会对count(*)做优化，如果有多个二级索引会使用key_len最小的进行扫描，如果没有二级索引会采用主键索引统计

>  *InnoDB handles SELECT COUNT(`\*`) and SELECT COUNT(`1`) operations in the same way. There is no performance difference.* 

不要使用count(字段)，效率最差，会采用全表扫描，因为要把每个字段取出来判断是否为null

为什么要通过遍历来统计？

myisam的特点是不支持事务，所以它可以在每个表简历meta信息存储count值，由表级锁保证一致性，而innoDb有事务，多版本并发控制，同时多个事务做insert或delete操作无法统计最终的count值，只能遍历

对count的优化，竟然还能使用explain的rows，但是这个值是估算值，就是不是很准，而且如果where条件中的字段没有索引，差别会非常大，剩下的就是维护一个单独的字段保存记录数



### MVCC,redolog,undolog,binlog

[mvcc讲的比较详细的](https://blog.csdn.net/SnailMann/article/details/94724197)
...
大概2000多万，计算方式，mysql最小存储单元为16k，假设一行数据大小为1k，单个叶子节点能存储的数据为16，如果主键id为bigint长度为8字节，而指针大小在InnoDB源码中设置为6字节，这样一共14字节，我们一个页中能存放多少这样的单元，其实就代表有多少指针，即16384/14=1170。那么可以算出一棵高度为2的B+树，能存放1170*16=18720条这样的数据记录。
...
https://www.cnblogs.com/wdss/p/11186411.html 索引失效
...
https://segmentfault.com/a/1190000008131735  explain
...
1、每个字段设定合适的大小，尽量减少行的size，为什么呢，因为对于innodb引擎，最小的存储单元是页（page）也就是16k，b+树是叶子节点保存数据，非叶子节点保存主键和指针，加一起是14字节，所以每个节点能保存1170条记录，每个记录指向一个page，每个page能放多少数据，能放的越少越好，所以需要设定合理的值
2、字段设置尽量不设置null，首先还是存储空间，需要一个额外的字节存储null标志位，引发索引问题，is null无法走索引，not in查询会返回永远为null的值

### Buffer Pool

[mysql buffer pool](https://blog.csdn.net/qq_35188937/article/details/109521846)

https://www.cnblogs.com/better-farther-world2099/articles/14768929.html

mysql的内存空间，放入的是数据页（16k）和描述数据表空间，buffer poll中的地址等

后续操作会直接操作buffer pool

- 查询：先查缓存，没有的话从磁盘加载到缓存，未来直接查缓存
- 插入：直接写缓存
- 更新删除：缓存中存在直接更新，不存在的话加载到缓存更新

Innodb提供WAL，通过redo log让mysql宕机后恢复数据，所以不会造成缓存中的数据丢失

**Buffer Pool初始化**

在mysql启动的时候，根据配置 innodb_buffer_pool_size 分配内存区域，按照数据页和描述文件，划分一个个空间，此时数据都是空的

**Free 链表**

运行起来后做CRUD操作，想知道哪些缓存页是空的，把这些信息放入free链表，主要通过描述文件添加free节点指针完成，写入数据后移出free链表

**缓存页hash表**

表空间号+数据页号作为 key，然后缓存页的地址作为 value。  每次加载数据页到空闲缓存页时，就写入一条映射关系到缓存页哈希表中。 

**Flush 链表**

空闲时有异步线程把缓存刷到磁盘，更新过的缓存页描述文件增加flush节点和节点指针，刷新是从Flush 链表查找数据，刷新完移出链表

**LRU 链表**

首先新增LUR节点，在描述文件里增加对下一个LRU节点的引用，只要是近期访问过的数据就放在链表头部，淘汰链表尾部的数据即可

**LRU 链表优化**

但有两个场景会导致热数据被淘汰， **预读机制**和**全表扫描**，局部性原理，会把相邻的数据也加载进来，到这些数据可能没用到过，全表扫描，大量的数据把空闲缓存用完，并且这些数据在前面，导致LRU把热数据清理也叫缓冲池污染。

我们需要给 LRU 链表做冷热数据分离设计，把 LRU 链表按一定比例，分为冷热区域。热区域称为 young 区域，冷区域称为 old 区域。 

 ![图片](https://mmbiz.qpic.cn/mmbiz_png/23OQmC1ia8nzibSg1dO1RTZY5uYMEsAgSzk1DKfvTzGyQVv6ZKWmR7cFRSFy3B2VWNqksred9hUjw7vWic1iayUMjA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1) 
...
1. JDK的命令行工具
    ...
    * jmap(Java内存映像工具)：命令用于生成堆转储快照(一般称为heapdump或dump文 件)。如果不使用jmap命令，要想获取Java堆转储快照，还有一些比较“暴力”的手段:譬如 在第2章中用过的-XX:+HeapDumpOnOutOfMemoryError参数，可以让虚拟机在OOM异常出 现之后自动生成dump文件。jmap的作用并不仅仅是为了获取dump文件，它还可以查询finalize执行队列、Java堆和永 久代的详细信息，如空间使用率、当前用的是哪种收集器等。jmap -dump:live,format=b,file=dump.hprof
...
1. new
2. Class类的newInstance（反射），(Student)Class.forName("Student类全限定名").newInstance() 这里调用的是无参的构造器
3. Constructor类的newInstance方法（反射），Student.class.getConstructor(Integer.class).newInstance(123)，可以调用有参数和私有的构造函数
4. clone方法，这个过程不会调用构造方法
5. 反序列化，这个过程不会调用构造方法

### list遍历相关

Java中Collections.sort()方法，java7没有问题，java8会出问题

[list遍历导致ConcurrentModificationException异常](https://cloud.tencent.com/developer/article/1125309)



### HashMap1.7和1.8的区别

1. 1.7使用头插法，1.8使用尾插法，解决死循环问题 [hashmap环链表](https://blog.csdn.net/thqtzq/article/details/90485663)
2. 1.7使用链表，1.8使用红黑树
...
允许一个或多个线程一直等待，直到其他线程操作执行完毕再执行。底层实现有个计数器，当计数器为0的时候可以继续执行
...
栅栏，计数器可重用，只有调用await的次数达到计数时，await方法才会继续执行，否则阻塞
...
前三种方式加的是对象锁，但是如果（2）中obj是一个class类型的对象，那么加的是类锁，

并且锁的范围比（4）还要大；如果该class类型的某个实例对象获得了类锁，那么该class类型的所有实例对象的synchronized代码块均无法访问。
...
终于搞明白了，spi是一种机制，一种由服务商提供接口，由使用者提供实现，来达到扩展的目的，通过引用包无需变更代码。还有spi是一种规范，要在META-INF/service/下创建类的全限定名的文件，然后一个关键的类ServiceLoader方法来获取对应的实现列表

### 双亲委派模型

定义就是加载class不是自己加载，而是问父加载器，父加载器加载不了再由当前加载器加载

总共3种加载器，boot启动类加载器是最顶层jre/lib，后面是ext扩展类加载器jre/lib/ext，后面是app应用加载器classpath

好处：可以避免类的重复加载，安全，加载核心类时区lib下面，这种类一般不会被替换，防止核心api被篡改

### 泛型的类型擦除

定义，虽然定义了泛型，但是在编译时检测生效，在编译后都会失去原有的泛型定义

1. <T>会被替换成Object
2. <T extends Integer>会被替换成Integer
3. 泛型有限制时会被擦除为上界或下界public <T extends Integer>T getWeight(T val)   ->  public Integer getWeight(Integer val)
4. 可以通过反射放进去不是泛型定义的类型，但获取的时候会失败，应该会做强制转换，到时候就是cannot be cast to 异常。
5. 编译器通过桥接方法，满足泛型的多态性，就是定义了 T get(T param) 接口，实现了Integer get(Integer param) 这样实现与接口的方法不一致
...
1. @Transactional 在非public方法上：当标记在protected、private、package-visible方法上时，不会产生错误，但也不会表现出为它指定的事务配置。可以认为它作为一个普通的方法参与到一个public方法的事务中。
2. @Transactional 的事务传播方式配置错误。 PROPAGATION_SUPPORTS 、 PROPAGATION_NOT_SUPPORTED 、 PROPAGATION_NEVER  
3. @Transactional 注解属性 rollbackFor 设置错误：Spring默认抛出了未检查unchecked异常（继承自 RuntimeException 的异常）或者 Error才回滚事务；其他异常不会触发回滚事务。 抛出的异常是 rollbackFor 指定的异常的子类，事务同样会回滚。 
4. 同一个类中方法调用，导致@Transactional失效：由于使用Spring AOP代理造成的，因为只有当事务方法被当前类以外的代码调用时，才会由Spring生成的代理对象来管理。 方法A没有声明注解事务，而B方法有。则外部调用方法A之后，方法B的事务是不会起作用的。 
...
spring主要的功能就是提供对bean的创建和管理，首先会创建BeanFactory，

这是bean的容器，然后创建BeanPostProcessor对bean进行实例化实现类，AwareBeanPostProcessor实现类是在执行bean实例化阶段前后进行调用，BeanPost是在初始化阶段前后

![bean生命周期中最重要的两个方法](https://upload-images.jianshu.io/upload_images/4558491-dc3eebbd1d6c65f4.png?imageMogr2/auto-orient/strip|imageView2/2/w/823/format/webp)

![bean生命周期](https://images0.cnblogs.com/i/580631/201405/181453414212066.png)

![bean生命周期](https://images0.cnblogs.com/i/580631/201405/181454040628981.png)
...
[Springboot稳定版本选择](https://blog.csdn.net/haiertadiedie/article/details/109773036)
...
基于注解，方法执行时是由指定的线程池中的线程执行，spring默认线程池为simpleAsyncTaskExecutor，该线程池默认来一个任务创建一个线程，若不断创建线程，最终会导致内存过高，引发oom，有限流开关 concurrencyLimit。可以实现自定义的线程池 AsyncConfigurer 

## 消息队列Kafka

[设计原理详解](https://blog.csdn.net/suifeng3051/article/details/48053965)
...
Kafka最初考虑的问题是，customer应该从brokes拉取消息还是brokers将消息推送到consumer，也就是pull还push。在这方面，Kafka遵循了一种大部分消息系统共同的传统的设计：producer将消息推送到broker，consumer从broker拉取消息。push模式下，当broker推送的速率远大于consumer消费的速率时，consumer恐怕就要崩溃了。最终Kafka还是选取了传统的pull模式。Pull模式的另外一个好处是consumer可以自主决定是否批量的从broker拉取数据。Pull有个缺点是，如果broker没有可供消费的消息，将导致consumer不断在循环中轮询，直到新消息到达。为了避免这点，Kafka有个参数可以让consumer阻塞直到新消息到达。

### Kafka是如何实现高吞吐率的

1. 顺序读写：kafka的消息是不断追加到文件中的，这个特性使kafka可以充分利用磁盘的顺序读写性能
2. 零拷贝：跳过“用户缓冲区”的拷贝，建立一个磁盘空间和内存的直接映射，数据不再复制到“用户态缓冲区”，一个是producer生产数据到broker，二是consumer从broker读取数据
...
零拷贝的定义就是把数据直接在内存上做映射，节省不必要的拷贝次数，底层还是linux mmap方式

正常的读取文件方式需要4步

1. 磁盘文件读取到操作系统内核缓冲区
2. 内核缓冲区copy到应用程序的内存
3. 将程序内存中的数据copy到socket网络发送缓冲区
4. 讲socket数据copy到网卡，由网卡传输
...
对于https，私钥放在服务器上，公钥是公开的，一串加密的字符串，加密传输信息的。https采用的对称加密和非对称加密的混合加密模式，通信建立前采用非对称加密，交换会话秘钥，通信过程中采用对称加密明文数据

- 对称加密：只是用一个密钥，运算速度快，密钥必须保密
- 非对称加密：使用两个密钥，公钥和私钥

https的4次通信

1. 客户端发起，client hello，传一个随机数，并且给服务端一个加密列表，大哥你看这是我支持的加密方式，咱用哪个？
2. 服务端返回server hello，也有个随机数，还有一个选择的加密方式，爷用这个加密吧
3. 服务端还返回一个服务端的公钥，这是爷用的密码，你拿去用吧
4. 客户端拿到公钥，对服务器端随机数进行加密，在用公钥加密，发送服务器端，大哥我用这个加密完了，你看下对不对
5. 服务端拿到后，用自己的私钥解密，对比，正确之后可以进行通信 
...
[三次握手和四次握手](https://blog.csdn.net/hyg0811/article/details/102366854)
...
### 四次挥手
 ...
 ![img](https://img-blog.csdn.net/20180720212258640?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM2MTMyMTI3/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70) 
...
“停止等待”就是每发送完一个分组就停止发送，等待对方确认后再发送下一个分组，A发送完一个分组M1后要等待B返回确认分组M1，如果发送超时或者接受B确认超时都会重新发分组。

1. A发送分组后要保留发送的副本，作为超时重传使用，收到该分组的确认后删除副本
2. 分组和确认分组都有编号，这样才能对应上
3. 超时计时器设置重传时间，要谨慎。

### TCP连接未立即释放

一个优化场景，qps到2.2w，TCP连接数到1.7w，虽然socket连接数足够，但并没有全部被用上，猜测每次请求后，tcp连接没有立即释放，导致socket无法重用
...
[tomcat官网所有配置](https://tomcat.apache.org/tomcat-7.0-doc/config/http.html)
...
1. 策略模式，一个接口，多个实现，当使用时需自行决定使用哪个策略类，使用策略类消除if-else
2. 模板方法，一个抽象类，里面有几个方法的实现，是通用的，但是不用的业务有些方法是不用的，把不同的方法放在子类实现

## 工作总结

### 多系统联合开发注意事项

1. 对数据库的映射对象，使用项目中自己定义的，不用共用
2. 返回的应答报文应该越详细越好，如子游戏未到止售时间，计奖服务未启动
3. 定规范，统一异常处理，PO，DO，VO，entity，DTO
4. 统一request，response，异常码返回
5. 复杂系统要写流程图，设计文档

## review哪些关注点

1. 对于review大部分的格式问题，没有注释问题，map没有初始化问题，都已经解决了，因为使用了checkstyle和findbugs
2. 初始化的大小，比如线程池，redis连接池，kafka连接池，feign连接池配置的是否合理
3. 关键业务是否有报警，是否对远程调用配置降级
...
1. 计数器：一段时间能访问资源的次数，最简单的限流方式，可配置单机、集群、接口的限流
2. 漏铜：保证一定的速率，桶有一定的容量，超过后拒绝，无法应对一些突发流量
3. 令牌桶：一边生成令牌放进桶里，请求过来会拿走令牌，能应对一些突发流量
4. 滑窗法：把时间分成很多段，每一段统计这个时间的请求数量，然后统计再一段时间内的总数
